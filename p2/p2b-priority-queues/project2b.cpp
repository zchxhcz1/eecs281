...
void testHiddenData() {
    ...
    // above.
        PQ<HiddenData, HiddenDataComp> hiddenPQ;
    hiddenPQ.push(HiddenData{10});
    hiddenPQ.push(HiddenData{20});
    hiddenPQ.push(HiddenData{5});

    // 验证堆操作
    std::cout << "Top element in HiddenData PQ: " << hiddenPQ.top().data << std::endl;  // 应该是 5

    hiddenPQ.pop();
    std::cout << "Top element in HiddenData PQ after one pop: " << hiddenPQ.top().data << std::endl;  // 应该是 10

    hiddenPQ.pop();
    std::cout << "Top element in HiddenData PQ after two pops: " << hiddenPQ.top().data << std::endl;  // 应该是 20

    hiddenPQ.pop();
    std::cout << "Is HiddenData PQ empty? " << (hiddenPQ.empty() ? "Yes" : "No") << std::endl;  // 应该是 Yes
...
template <>
void testPriorityQueue<SortedPQ>() {
    testPrimitiveOperations<SortedPQ>();
    testHiddenData<SortedPQ>();
    testUpdatePriorities<SortedPQ>();
}

template <>
void testPriorityQueue<BinaryPQ>() {
    testPrimitiveOperations<BinaryPQ>();
    testHiddenData<BinaryPQ>();
    testUpdatePriorities<BinaryPQ>();
}
