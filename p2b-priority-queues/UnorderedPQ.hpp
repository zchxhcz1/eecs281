...
public:
    ...
    virtual ~UnorderedPQ() = default;


    // Description: Copy constructors don't need any code, the data vector
    //              will be copied automatically.
    UnorderedPQ(const UnorderedPQ &) = default;
    UnorderedPQ(UnorderedPQ &&) noexcept = default;


    // Description: Copy assignment operators don't need any code, the data
    //              vector will be copied automatically.
    UnorderedPQ &operator=(const UnorderedPQ &) = default;
    UnorderedPQ &operator=(UnorderedPQ &&) noexcept = default;
    ...
    virtual void push(const TYPE &val) { data.push_back(val); }
...
private:
    ...
    //              another.
    // Runtime: O(n)
    [[nodiscard]] size_t findExtreme() const {
        size_t index = 0;
